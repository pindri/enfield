...
Each script runs a sweep of `toy.py` and then calls `collect_reports.py` to aggregate the JSON outputs into a CSV under `analysis/`.
...
| `run_fmnist_main_mechanism.sh` | FashionMNIST main sweep (also produces three-panel figure) |
| `cifar100_test.sh` | CIFAR100 diagnostic |
| `run_cifar100_diagnostic_tinycnn.sh` | CIFAR100 diagnostic with explicit TinyCNN |
| `fmnist_test.sh` | FashionMNIST diagnostic |

```bash
bash run_main_mechanism.sh
# ... repeat for whichever experiments you need
```
...
After running the main CIFAR10 experiments, generate all plots into `analysis/main/`:

```bash
python make_plots.py
```

For the FashionMNIST three-panel figure specifically:
...
Edit the `grid` and `contract` in `compiler.py:main()` to change the search space and requirements. Results are saved to `contracts/`.
